...
def handler(request):
    ...
    if request.method == 'POST':
        ...
        if path == '/generate' or path == '/api/generate':
            ...
            if not uid or not password:
                return generate_response(400, {"error": "UID and password are required"})
            
            result = generate_jwt(uid, password)
            status_code = 200 if result["success"] else 400
            
            return generate_response(status_code, result)
        
        # Generate batch
        if path == '/generate_batch' or path == '/api/generate_batch':
            accounts = body.get('accounts', [])
            
            if not accounts:
                return generate_response(400, {"error": "No accounts provided"})
            
            # Limit batch size to prevent timeout
            if len(accounts) > 50:
                return generate_response(400, {"error": "Maximum 50 accounts per batch"})
    ...
    return generate_response(405, {"error": "Method not allowed"})    return cipher.encrypt(padded)
